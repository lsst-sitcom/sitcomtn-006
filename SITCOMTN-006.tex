...
\begin{tabular}{ll|c|c|p{0.29\textwidth}|p{0.25\textwidth}}
  ...
  \hline
  \IM{a} & \IMsummary{a} & N & \IMdate{a} & \textit{c.} 2021-05-01 & \\
  \IM{c} & \IMsummary{c} & N & \IMdate{c} & \textit{c.} 2021-06 & no logging \\
  \IM{ppg} & \IMsummary{ppg} & Y & \IMdate{ppg} & \textit{c.} 2021-06-24 & In parallel with \IM{f} \\
  \IM{b} & \IMsummary{b} & N & \IMdate{b} & 2021-08-01 only biases & no logging \\
  \IM{d} & \IMsummary{d} & Y & \IMdate{d} & 2021-08-05 & \\
  \IM{pg} & \IMsummary{pg} & Y & \IMdate{pg} & 2021-08-19 (Tiago to provide docs) & In parallel with \IM{f} \\
  \IM{ppf} & \IMsummary{ppf} & Y & \IMdate{ppf} & 2021-08-19 (if all 4 corner rafts OK) & \\
  \IM{pf} & \IMsummary{pf} & Y & \IMdate{pf} & & In parallel with \IM{pg} \\
  \IM{f} & \IMsummary{f} & Y & \IMdate{f} & & In parallel with \IM{g} \\
  \IM{g} & \IMsummary{g} & Y & \IMdate{g} & & Blocked by DAQ playback \\
  \IM{e} & \IMsummary{e} & N & \IMdate{e} & & \\
  \IM{h} & \IMsummary{h} & Y & \IMdate{h} & & Blocked by DAQ playback \\
  \IM{pm} & \IMsummary{pm} & Y/N & \IMdate{pm} & & \\
  \IM{m} & \IMsummary{m} & N & \IMdate{m} & & \\
  \IM{n} & \IMsummary{n} & N & \IMdate{n} & & \\
  \IM{o} & \IMsummary{o} & N & \IMdate{o} & & \\
  \hline
  \IM{i} & \IMsummary{i} & Y & \IMdate{i} & & \\
  \IM{j} & \IMsummary{j} & N & \IMdate{j} & & \\
  \IM{k} & \IMsummary{k} & Y & \IMdate{k} & & \\
  \IM{cbp} & \IMsummary{cbp} & N & \IMdate{cbp} & & \\
\end{tabular}
